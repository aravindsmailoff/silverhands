...
def detect_scenes(video_path):
    ...
    if abs_path in _SCENE_CACHE:
        return _SCENE_CACHE[abs_path]

    import torch
    has_cuda = torch.cuda.is_available()
    engine = os.environ.get("SCENE_ENGINE", "auto").strip().lower()

    # TransNetV2 is only fast when GPU acceleration (CUDA) is available.
    # On CPU, PySceneDetect ContentDetector is 30x faster (seconds vs minutes).
    use_transnet = (engine == "transnetv2") or (engine == "auto" and has_cuda)

    if use_transnet:
        try:
            res = _detect_transnetv2(video_path)
            _SCENE_CACHE[abs_path] = res
            return res
        except Exception as e:
            print(f"   ⚠️ TransNetV2 scene detection failed "
                  f"({type(e).__name__}: {e}) — falling back to fast PySceneDetect")
    
    print(f"   🎬 Scene engine: Fast PySceneDetect (ContentDetector)")
