...
ANALYSIS_MAX_WIDTH = int(os.environ.get("REFRAME_ANALYSIS_WIDTH", "640"))
...
def render(input_video, final_output_video, aspect_ratio, content_ranges=None):
    ...
    try:
        ...
        def _render_one_scene(idx, start_f, end_f, strategy):
            ...
            if strategy == 'INSET':
                ...
                # Use a relative path to avoid the drive letter colon which breaks ffmpeg filter parsing
                cmd_path_ffmpeg = os.path.relpath(cmd_path).replace("\\", "/")
                graph = (
                    f"[0:v]sendcmd=f='{cmd_path_ffmpeg}',"
                    f"crop@c={init},"
        ...
        max_workers = max(1, min(int(os.environ.get("REFRAME_SCENE_WORKERS", "4")), len(ranges)))
        results = [None] * len(ranges)
        with ThreadPoolExecutor(max_workers=max_workers) as pool:
            futs = [pool.submit(_render_one_scene, idx, s_f, e_f, strat) for idx, (s_f, e_f, strat) in enumerate(ranges)]
            for f in as_completed(futs):
                idx, seg_path = f.result()
                results[idx] = seg_path
        segments = results
