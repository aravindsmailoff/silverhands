...
def _offline_viral_clips(transcript_result, video_duration, words):
    """Offline fallback for viral clip selection when Gemini API is unavailable."""
    print("🎬 Generating offline viral clips from transcript windows...")
    windows = build_transcript_windows(transcript_result, video_duration) if transcript_result.get("segments") else []
    if not windows:
        clip_dur = min(video_duration, 45.0)
        return {
            "shorts": [{
                "start": 0.0,
                "end": clip_dur,
                ...
                "predicted_score": 85,
                "video_description_for_tiktok": "Highlights from video",
                "video_description_for_instagram": "Highlights from video",
                "video_title_for_youtube_short": "Top Highlight",
                "viral_hook_text": ""
            }]
        }
    
    shorts = []
    step = max(1, len(windows) // 3)
    for i in range(0, len(windows), step):
        w = windows[i]
        ns, ne = snap_clip_to_words(w["start"], w["end"], words, video_duration) if words else (w["start"], w["end"])
        shorts.append({
            "start": ns,
            "end": ne,
            "source_window_id": w["id"],
            "predicted_score": 85,
            "video_description_for_tiktok": f"Highlight: {w.get('text', '')[:50]}",
            "video_description_for_instagram": f"Highlight: {w.get('text', '')[:50]}",
            "video_title_for_youtube_short": f"Highlight #{len(shorts) + 1}",
            "viral_hook_text": ""
        })
        if len(shorts) >= 3:
            break
...
def _offline_silverhands_showcase(transcript_result, video_duration, focus_guidance="", mode="highlight", vuo=None, video_path=None):
    """
    Offline/Fallback showcase assembler when Gemini API is offline or quota exceeded.
    Constructs a 30-60s narrative arc (Hook -> Demonstration -> Payoff -> Outro).
    """
    print("🎬 Generating smart offline editorial showcase plan...")
    segments_raw = transcript_result.get("segments", []) if transcript_result else []
    
    plan_segments = []
    
    if segments_raw and len(segments_raw) >= 1:
        total_segs = len(segments_raw)
        
        # 1. Opening Hook (first 5 - 10s)
        first_seg = segments_raw[0]
        hook_end = min(video_duration * 0.3, max(first_seg.get("end", 5.0), 6.0))
        plan_segments.append({
            "start": float(first_seg.get("start", 0.0)),
            "end": float(hook_end),
            "semantic_start": float(first_seg.get("start", 0.0)),
            "semantic_end": float(hook_end),
            "role": "opening",
            "beat": "hook",
            "label": "Opening Hook",
            "topic": "Introduction",
            "speed": 1.0,
            "transition_type": "cut",
            "transition_in_reason": "Video opening",
            "reason": "Direct hook introducing the main idea",
            "contains_speech": True,
            "speaker_visible": True,
            "showcase_value": 90
        })
        
        # 2. Demonstration / Core technique (~15-25s)
        if total_segs > 2 and video_duration > 15.0:
            mid_start_time = video_duration * 0.35
            mid_end_time = min(video_duration * 0.75, mid_start_time + 20.0)
            plan_segments.append({
                "start": float(mid_start_time),
                "end": float(mid_end_time),
                "semantic_start": float(mid_start_time),
                "semantic_end": float(mid_end_time),
                "role": "core_showcase",
                "beat": "demonstration",
                "label": "Core Technique",
                "topic": "Process & Demonstration",
                "speed": 1.0,
                "transition_type": "cut",
                "transition_in_reason": "Jump to core action",
                "reason": "Detailed hands-on demonstration",
                "contains_speech": True,
                "speaker_visible": True,
                "showcase_value": 95
            })
            
        # 3. Climax / Payoff (~10-15s)
        if video_duration > 20.0:
            end_start_time = max(video_duration * 0.75, video_duration - 15.0)
            end_time = video_duration
            plan_segments.append({
                "start": float(end_start_time),
                "end": float(end_time),
                "semantic_start": float(end_start_time),
                "semantic_end": float(end_time),
                "role": "closing",
                "beat": "payoff",
                "label": "Final Result",
                "topic": "Conclusion & Reveal",
                "speed": 1.0,
                "transition_type": "cut",
                "transition_in_reason": "Jump to final reveal",
                "reason": "Satisfying outcome and conclusion",
                "contains_speech": True,
                "speaker_visible": True,
                "showcase_value": 92
            })
        ...
        plan_segments = [
            {
                "start": 0.0,
                "end": min(video_duration, quarter),
                "semantic_start": 0.0,
                "semantic_end": min(video_duration, quarter),
                "role": "opening",
                "beat": "hook",
                "label": "Visual Hook",
                "topic": "Opening",
                "speed": 1.0,
                "transition_type": "cut",
                "transition_in_reason": "Video start",
                "reason": "Visual opening hook",
                "contains_speech": False,
                "showcase_value": 90
            },
            {
                "start": quarter,
                "end": min(video_duration, quarter * 3),
                "semantic_start": quarter,
                "semantic_end": min(video_duration, quarter * 3),
                "role": "core_showcase",
                "beat": "demonstration",
                "label": "Action Showcase",
                "topic": "Craft Demonstration",
                "speed": 1.0,
                "transition_type": "cut",
                "transition_in_reason": "Action cut",
                "reason": "Core demonstration",
                "contains_speech": False,
                "showcase_value": 95
            },
            {
                "start": min(video_duration, quarter * 3),
                "end": video_duration,
                "semantic_start": min(video_duration, quarter * 3),
                "semantic_end": video_duration,
                "role": "closing",
                "beat": "payoff",
                "label": "Final Result",
                "topic": "Reveal",
                "speed": 1.0,
                "transition_type": "cut",
                "transition_in_reason": "Outcome cut",
                "reason": "Final reveal",
                "contains_speech": False,
                "showcase_value": 92
            }
    ...
    title = focus_guidance.strip() if focus_guidance else (vuo.get("subject", "Craft Showcase") if vuo else "Craft & Skill Showcase")
    return {
        "showcase_title": title,
        "segments": plan_segments,
        "pass1_full_map": [],
        "opening_rationale": "Direct opening hook introducing the main craft or technique.",
        "closing_rationale": "Satisfying payoff and completed result.",
        "segments_rejected_and_why": "Low-energy dead space trimmed for maximum viewer engagement.",
        "final_editorial_explanation": "Coherent vertical short constructed using smart timeline pacing.",
        "cost_analysis": {"input_tokens": 0, "output_tokens": 0, "total_cost": 0.0, "model": "offline-heuristic"}
    }
...
def get_viral_clips(transcript_result, video_duration):
    ...
    if not api_key:
        print("⚠️ GEMINI_API_KEY not set — using offline viral clip selection.")
        return _offline_viral_clips(transcript_result, video_duration, words)
    ...
    try:
        ...
        if not shorts:
            print("⚠️ 2-pass returned no clips — falling back to offline selection.")
            return _offline_viral_clips(transcript_result, video_duration, words)
        ...
        print(f"⚠️ Gemini clip selection error: {e} — falling back to offline selection.")
        return _offline_viral_clips(transcript_result, video_duration, words)


def get_silverhands_showcase(transcript_result, video_duration, focus_guidance="", mode="highlight", vuo=None, video_path=None):
    """Two-pass Narrative Selection layer to generate one coherent showcase video.
    Pass 1 analyzes and classifies narrative roles and importances of all windows.
    Pass 2 designs the timeline layout from a temporally-balanced subset of Pass 1 windows.
    """
    print("🤖  Analyzing with SilverHands (2-pass: stage classification → showcase plan)...")
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
        print("⚠️ GEMINI_API_KEY not found — using offline showcase planner.")
        return _offline_silverhands_showcase(transcript_result, video_duration, focus_guidance, mode, vuo, video_path)

    client = genai.Client(api_key=api_key)
    model_name = os.environ.get("GEMINI_MODEL") or 'gemini-3.1-flash-lite'
    language = str(transcript_result.get('language') or 'unknown')
    print(f"🤖  Model: {model_name} | language: {language} | mode: {mode} | focus: {focus_guidance}")

    # Build windows from transcript or visual beats
    try:
        windows = build_transcript_windows(transcript_result, video_duration) if transcript_result.get("segments") else []
        if not windows:
            # Generate visual windows from VUO visual_beats if available, or uniform timeline windows
            if vuo and vuo.get("visual_beats"):
                windows = [{
                    "id": f"vbeat_{i+1}",
                    "start": float(b.get("start", 0)),
                    "end": float(b.get("end", 0)),
                    "text": f"[{b.get('stage', 'demonstration').upper()}]: {b.get('action', '')}"
                } for i, b in enumerate(vuo.get("visual_beats", []))]
                print(f"   Built {len(windows)} visual beat window(s) from VUO.")
            else:
                win_size = 8.0
                windows = []
                curr = 0.0
                ...
                while curr < video_duration:
                    w_end = min(video_duration, curr + win_size)
                    windows.append({
                        "id": f"vwin_{w_idx}",
                        "start": round(curr, 2),
                        "end": round(w_end, 2),
                        "text": f"[Visual craft demonstration #{w_idx}]"
                    })
                    curr += win_size
                    w_idx += 1
                print(f"   Built {len(windows)} visual timeline window(s).")
        else:
            print(f"   Built {len(windows)} narrative window(s).")
        costs = []

        # --- Pass 1: map/classify the entire transcript's windows ---
        scored = []
        SCORE_BATCH = 8
        for b in range(0, len(windows), SCORE_BATCH):
            batch = windows[b:b + SCORE_BATCH]
            payload = [{"id": w["id"], "start": w["start"], "end": w["end"], "text": w["text"]} for w in batch]
            prompt = gemini_worker.SILVERHANDS_PASS1_PROMPT_TEMPLATE.format(
                video_duration=video_duration, language=language, focus_guidance=focus_guidance,
                windows_json=json.dumps(payload, ensure_ascii=False))
            parsed, cost = _run_gemini_stage(client, model_name, prompt, gemini_worker.Pass1Response)
            if cost:
                costs.append(cost)
            scored.extend(parsed.get("windows") or [])

        print(f"   Mapped {len(scored)} window(s) via Pass 1.")

        # Temporal balancing: group scored windows into 4 chronological zones
        # and shortlist top windows by importance from each zone to feed Pass 2.
        zone_bounds = [
            (0.0, video_duration * 0.25),
            (video_duration * 0.25, video_duration * 0.5),
            (video_duration * 0.5, video_duration * 0.75),
            (video_duration * 0.75, video_duration)
        ]
        
        shortlist = []
        by_id = {w["id"]: w for w in windows}
        scored_by_id = {w["id"]: w for w in scored}
        
        for low_b, high_b in zone_bounds:
            zone_candidates = []
            for w in windows:
                mid = (w["start"] + w["end"]) / 2.0
                if low_b <= mid < high_b:
                    s_w = scored_by_id.get(w["id"])
                    if s_w:
                        zone_candidates.append(s_w)
            zone_candidates.sort(key=lambda x: x.get("importance", 0), reverse=True)
            for w in zone_candidates[:3]:
                if w["id"] in by_id:
                    shortlist.append(by_id[w["id"]])
                    
        shortlist.sort(key=lambda x: x["start"])
        print(f"   Shortlisted {len(shortlist)} temporally balanced window(s) for showcase design.")

        # --- Pass 2: Design showcase plan from the shortlist ---
        payload = []
        for w in shortlist:
            s_w = scored_by_id.get(w["id"]) or {}
            payload.append({
                "id": w["id"],
                "start": w["start"],
                "end": w["end"],
                "duration": round(w["end"] - w["start"], 2),
                "role": s_w.get("role", "explanation"),
                "importance": s_w.get("importance", 50),
                "showcase_value": s_w.get("showcase_value", 50),
                "narrative_necessity": s_w.get("narrative_necessity", "important"),
                "thought_boundary_start_phrase": s_w.get("thought_boundary_start_phrase", ""),
                "thought_boundary_end_phrase": s_w.get("thought_boundary_end_phrase", ""),
                "text": w["text"]
            })

        prompt = gemini_worker.SILVERHANDS_PASS2_PROMPT_TEMPLATE.format(
            video_duration=video_duration, language=language, focus_guidance=focus_guidance,
            windows_json=json.dumps(payload, ensure_ascii=False))
        detail, cost = _run_gemini_stage(client, model_name, prompt, gemini_worker.SilverHandsShowcaseResponse)
        if cost:
            costs.append(cost)
        ...
        # Enforce speed policies
        for s in segments:
            lbl = s.get("label", "unknown")
            if s.get("contains_speech") or s.get("speaker_visible") or s.get("role") in ("opening", "closing"):
                old_sp = s.get("speed", 1.0)
                if old_sp > 1.1:
                    print(f"   ⚠️ Pacing Guard: Segment '{lbl}' contains speech/talking face/hook/outro. Forcing speed from {old_sp}x to 1.0x.")
                    s["speed"] = 1.0
            elif s.get("visual_action_type") == "hand_close_up" or s.get("role") == "core_showcase":
                old_sp = s.get("speed", 1.0)
                if s.get("visual_action_type") == "hand_close_up" and old_sp > 1.1:
                    print(f"   ⚠️ Pacing Guard: Core manual Close-up segment '{lbl}'. Capping speed from {old_sp}x to 1.1x.")
                    s["speed"] = 1.1
                elif old_sp > 1.5:
                    print(f"   ⚠️ Pacing Guard: Core showcase segment '{lbl}'. Capping speed from {old_sp}x to 1.5x.")
                    s["speed"] = 1.5

        result = {
            "showcase_title": detail.get("showcase_title", "Showcase"),
            "segments": segments,
            "pass1_full_map": scored,
            "opening_rationale": detail.get("opening_rationale", ""),
            "closing_rationale": detail.get("closing_rationale", ""),
            "segments_rejected_and_why": detail.get("segments_rejected_and_why", ""),
            "final_editorial_explanation": detail.get("final_editorial_explanation", "")
        }
        if cost_analysis:
            result["cost_analysis"] = cost_analysis
        return result
    except Exception as e:
        print(f"⚠️ SilverHands Gemini Error: {e} — falling back to smart offline showcase assembler.")
        return _offline_silverhands_showcase(transcript_result, video_duration, focus_guidance, mode, vuo, video_path)


def resolve_segment_overlaps(segments):
    """Detects and resolves overlapping segments chronologically by merging them."""
    if not segments:
        return segments
    ...
    for s in segments:
        ...
        prev = resolved[-1]
        # Check if current segment overlaps with previous segment
        if s["start"] < prev["end"] - 0.1:
            print(f"   ⚠️ Overlap Detected: '{prev.get('label') or prev.get('topic')}' [{prev['start']:.2f}s -> {prev['end']:.2f}s] "
                  f"overlaps with '{s.get('label') or s.get('topic')}' [{s['start']:.2f}s -> {s['end']:.2f}s].")
            
            # Merge: extend prev['end'] to cover both
            prev["end"] = max(prev["end"], s["end"])
            # Update metadata to reflect the merge
            prev["label"] = f"{prev.get('label')} + {s.get('label')}"
            prev["topic"] = f"{prev.get('topic')} & {s.get('topic')}"
            prev["showcase_value"] = max(prev.get("showcase_value", 50), s.get("showcase_value", 50))
            print(f"   ✅ Overlap Resolved: Merged into continuous range [{prev['start']:.2f}s -> {prev['end']:.2f}s].")
        else:
            resolved.append(s)
...
def budget_and_snap_showcase_segments(segments, words, video_duration, target_min=30.0, target_max=60.0):
    """Snaps segment boundaries to words/scenes and budget-constrains the total duration to 30-60 seconds."""
    # First pass: Snap all segments
    for s in segments:
        if words and s.get("contains_speech", True):
            ns, ne = snap_clip_to_words(s.get("start", 0), s.get("end", 0), words, video_duration, min_duration=2.0)
            s["start"], s["end"] = ns, ne
        else:
            s["start"] = max(0.0, float(s.get("start", 0.0)))
            s["end"] = min(video_duration, float(s.get("end", s["start"] + 3.0)))

    # Resolve overlapping segment boundaries
    segments = resolve_segment_overlaps(segments)


    # Calculate post-snap total duration
    total_dur = sum((s["end"] - s["start"]) / s.get("speed", 1.0) for s in segments)
    print(f"   Post-snap initial total duration: {total_dur:.2f}s")

    # Smart trimming: Rank segments by drop preference
    # We rank by "keep strength":
    # 4 = Opening/Closing (Required, never drop)
    # 3 = Essential
    # 2 = Important
    # 1 = Supporting
    # 0 = Optional
    # Drop segments with lowest keep strength first; tie-breaker: drop lowest showcase value first.
    while total_dur > target_max and len(segments) > 3:
        candidates = []
        for idx, s in enumerate(segments):
            role = s.get("role", "")
            necessity = s.get("narrative_necessity", "important")
            val = s.get("showcase_value", 50)
            
            if role in ("opening", "closing"):
                strength = 4
            elif necessity == "essential":
                strength = 3
            elif necessity == "important":
                strength = 2
            elif necessity == "supporting":
                strength = 1
            else: # optional
                strength = 0
            candidates.append((idx, strength, val, s["end"] - s["start"]))
            
        # Sort so weakest candidate (most droppable) is at index 0
        candidates.sort(key=lambda x: (x[1], x[2]))
        
        # If the most droppable segment is critical (strength >= 4), we stop dropping to preserve start/end coherence.
        if candidates[0][1] >= 4:
            print("   Smart Trimming: All remaining segments are critical (opening/closing). Stopping dropping.")
            break
            
        drop_idx = candidates[0][0]
        dropped_lbl = segments[drop_idx].get("label") or segments[drop_idx].get("topic", "unlabeled")
        print(f"   Smart Trimming: Dropping segment '{dropped_lbl}' (strength={candidates[0][1]}, showcase_value={candidates[0][2]}) to reduce duration.")
        segments.pop(drop_idx)
        
        total_dur = sum((s["end"] - s["start"]) / s.get("speed", 1.0) for s in segments)
        print(f"   New duration after drop: {total_dur:.2f}s")

    # Fine-grained word-by-word trimming if still slightly over budget
    # Trim one word from the end of the longest segment at a time
    MAX_TRIM_ITERATIONS = 50
    for trim_iter in range(MAX_TRIM_ITERATIONS):
        if total_dur <= target_max:
            break
            
        # Find candidate segments with more than 3 words
        candidates = []
        for idx, s in enumerate(segments):
            seg_words = [w for w in words if s["start"] <= w["s"] < s["end"]]
            if len(seg_words) > 3:
                candidates.append((idx, len(seg_words), s["end"] - s["start"]))
        
        if not candidates:
            break
            
        # Sort candidates by current duration descending
        candidates.sort(key=lambda x: -x[2])
        target_idx = candidates[0][0]
        
        target_seg = segments[target_idx]
        seg_words = [w for w in words if target_seg["start"] <= w["s"] < target_seg["end"]]
        
        # Trim the last word from this segment
        target_seg["end"] = seg_words[-2]["e"]
        
        total_dur = sum((s["end"] - s["start"]) / s.get("speed", 1.0) for s in segments)

    print(f"   Duration after fine-grained word-by-word trimming: {total_dur:.2f}s")

    # Hard guard: Final safety fallback only
    if total_dur > target_max:
        print("   ⚠️ Hard boundary guard triggered: force-trimming absolute end of segments.")
        diff = total_dur - target_max
        for s in reversed(segments):
            if diff <= 0:
                break
            seg_dur = (s["end"] - s["start"]) / s.get("speed", 1.0)
            available_to_trim = seg_dur - 3.0 # keep at least 3s
            if available_to_trim > 0:
                trim_sec = min(diff, available_to_trim)
                s["end"] -= trim_sec * s.get("speed", 1.0)
                diff -= trim_sec
                
    total_dur = sum((s["end"] - s["start"]) / s.get("speed", 1.0) for s in segments)
    print(f"   Final budgeted showcase duration: {total_dur:.2f}s")
    return segments, total_dur


def auto_caption_showcase(clip_path, transcript, segments):
    """Burns word-level subtitles remapped to the continuous timeline of the concatenated showcase video."""
    if os.environ.get("AUTO_CAPTIONS", "1").strip() == "0":
        return None
    if not transcript or not transcript.get('segments'):
        return None  # silent video: nothing to caption
    try:
        ...
        # Remap word alignment timestamps using speed scaling and segment offsets
        remapped_transcript = remap_subtitles(transcript, segments)
        total_duration = sum((s['end'] - s['start']) / s.get('speed', 1.0) for s in segments)
        
        if not _subs.generate_ass(
                remapped_transcript, 0.0, total_duration, ass_path,
                max_chars=style["max_chars"], max_duration=style["max_duration"],
                alignment=style["alignment"], fontsize=style["font_size"],
                font_name=style["font_name"], font_color=style["font_color"],
                border_color=style["border_color"], border_width=style["border_width"],
                highlight_color=style["highlight_color"], effect=style["effect"],
                base_opacity=style["base_opacity"], uppercase=style["uppercase"]):
            print("   ℹ️ No words in range — showcase ships without captions.")
            return None
        ...
        print(f"   ⚠️ Showcase auto-captions failed ({type(e).__name__}: {e}) — delivering without them.")
        return None
...
if __name__ == '__main__':
    ...
    input_group.add_argument('--session-id', type=str, help="Persistent session ID to reuse cached video memory.")
    ...
    # 1. Get Input Video or Session Memory
    session_memory = None
    if args.session_id:
        import session_manager
        session_memory = session_manager.load_session_memory(args.session_id)
        if not session_memory:
            print(f"❌ Session memory not found for session ID: {args.session_id}")
            exit(1)
        input_video = session_memory["source_path"]
        video_title = os.path.splitext(os.path.basename(input_video))[0]
        output_dir = _ensure_dir(args.output) if args.output else session_memory["session_dir"]
    elif args.url:
    ...
    if args.skip_analysis:
        ...
        version_num = 1
        if session_memory:
            transcript = session_memory["transcript"]
            vuo = session_memory["vuo"]
            duration = float(session_memory.get("duration", 60.0))
            raw_scenes = session_memory.get("scenes", [])
            # Convert float scene cuts into list of tuples or pass directly
            scene_list = []
            if raw_scenes and isinstance(raw_scenes[0], (int, float)):
                # List of cut points in seconds
                scene_list = [(s, s) for s in raw_scenes]
            elif raw_scenes:
                scene_list = raw_scenes
            
            import session_manager
            version_num = session_manager.get_next_version_number(args.session_id)
            print(f"🎬 [NEW] Generating editorial plan (version {version_num}) for focus: '{args.focus}'...")
        else:
            ...
            import scene_detection
            try:
                scene_list, _ = scene_detection.detect_scenes(input_video)
            except Exception as e:
                print(f"   ⚠️ Scene detection skipped: {e}")
                scene_list = []

            # Build VUO if visual/mixed
            import gemini_worker
            content_mode = gemini_worker.classify_content_mode(transcript, duration)
            if content_mode in ("visual", "mixed"):
                print(f"👁️  Visual/Mixed content detected ({content_mode}) — running multimodal VUO...")
                api_key = os.getenv("GEMINI_API_KEY")
                if api_key:
                    client = genai.Client(api_key=api_key)
                    model_name = os.environ.get("GEMINI_MODEL") or 'gemini-3.1-flash-lite'
                    vuo, _ = gemini_worker.analyze_video_understanding(client, model_name, transcript, duration, video_path=input_video)

        if args.silverhands:
            showcase_data = get_silverhands_showcase(transcript, duration, focus_guidance=args.focus, mode=args.mode, vuo=vuo, video_path=input_video)
            if not showcase_data or 'segments' not in showcase_data:
                raise RuntimeError("SilverHands analysis failed to design a showcase.")

            
            # Extract words list from transcript for snapping
            words = []
            for segment in transcript.get('segments', []):
                for word in segment.get('words', []):
                    words.append({'w': word['word'], 's': word['start'], 'e': word['end']})
                    
            # Refine boundaries with deterministic ground truth (Whisper words + scene cuts + pacing rules + natural endings)
            import editorial_engine
            raw_segments = showcase_data['segments']
            
            # Format scenes for editorial engine
            processed_scenes = []
            if scene_list:
                for sc in scene_list:
                    if hasattr(sc[0], 'get_seconds'):
                        processed_scenes.append((sc[0].get_seconds(), sc[1].get_seconds()))
                    elif isinstance(sc, (int, float)):
                        processed_scenes.append((sc, sc))
                    elif isinstance(sc, (list, tuple)) and isinstance(sc[0], (int, float)):
                        processed_scenes.append((float(sc[0]), float(sc[1])))

            final_segments, total_showcase_duration = editorial_engine.refine_editorial_boundaries(
                raw_segments, words, scene_list, duration
            )
            
            showcase_data['segments'] = final_segments
            showcase_data['total_duration_seconds'] = total_showcase_duration
            
            # Map the showcase as the single "shorts" clip for out-of-the-box dashboard compatibility
            showcase_clip = {
                "start": 0.0,
                "end": total_showcase_duration,
                "video_title_for_youtube_short": showcase_data.get("showcase_title", "Showcase"),
                "video_description_for_tiktok": "SilverHands Showcase",
                "video_description_for_instagram": "SilverHands Showcase",
                "viral_hook_text": "",
                "source_window_id": "showcase"
            }
            showcase_data['shorts'] = [showcase_clip]
            showcase_data['transcript'] = transcript
            
            # Save metadata
            metadata_file = os.path.join(output_dir, f"{video_title}_metadata.json")
            with open(metadata_file, 'w', encoding='utf-8') as f:
                json.dump(showcase_data, f, indent=2, ensure_ascii=False)
            print(f"   Saved SilverHands metadata to {metadata_file}")
            
            if args.plan_only:
                print("\n==================================================")
                print("📋 SILVERHANDS V6 EDITORIAL PLAN (DRY-RUN)")
                print("==================================================")
                print(f"1. Showcase Title: {showcase_data.get('showcase_title')}")
                print(f"2. Total Planned Duration: {total_showcase_duration:.2f} seconds")
                print("\n3. Ordered Segment List & Beats:")
                for i, s in enumerate(final_segments):
                    print(f"   - Segment {i+1}: [{s['start']:.2f}s -> {s['end']:.2f}s] | role: {s.get('role')} | beat: {s.get('beat')} | speed: {s.get('speed')}x | cut: {s.get('transition_type', 'cut')} | topic: {s.get('topic')}")
                print(f"\n4. Opening Rationale:\n   {showcase_data.get('opening_rationale')}")
                print("\n5. Transition Rationale for Every Cut:")
                for i, s in enumerate(final_segments):
                    if i > 0:
                        print(f"   - Cut {i} -> {i+1} Transition In ({s.get('transition_type', 'cut')}): {s.get('transition_in_reason')}")
                    print(f"     Segment {i+1} Internal Flow: {s.get('reason')}")
                print(f"\n6. Closing Rationale:\n   {showcase_data.get('closing_rationale')}")
                print(f"\n7. Segments Rejected and Why:\n   {showcase_data.get('segments_rejected_and_why')}")
                print(f"\n8. Final Editorial Explanation:\n   {showcase_data.get('final_editorial_explanation')}")
                print("==================================================\n")
                sys.exit(0)
            
            # Render the showcase using Audio-Aware JL Cut Assembler
            import jl_cut_assembler
            
            # Compatible naming so app.py serves it as the first clip
            clip_filename = f"{video_title}_clip_1.mp4"
            intermediate_path = os.path.join(output_dir, f"temp_{clip_filename}")
            final_path = os.path.join(output_dir, clip_filename)
            
            print(f"🎬 Assembling {len(final_segments)} showcase segments into {intermediate_path} with J/L cut audio...")
            jl_cut_assembler.build_with_editorial_cuts(input_video, final_segments, intermediate_path)
            
            print(f"🚀 Reframing assembled video to {final_path} (Version {version_num})...")
            success = render_clip(intermediate_path, final_path, output_format)
            ...
            if success:
                if os.environ.get("WATERMARK") == "1":
                    apply_watermark(final_path)
                # Auto-caption showcase
                captioned_path = auto_caption_showcase(final_path, transcript, final_segments)
                if captioned_path:
                    print(f"✅ Showcase ready (with captions): {captioned_path}")
                else:
                    print(f"✅ Showcase ready (no captions): {final_path}")

                # If running within a session, record this version in persistent history
                if args.session_id:
                    import session_manager
                    session_manager.record_session_version(
                        args.session_id,
                        version_num,
                        args.focus,
                        showcase_data,
                        final_path,
                        total_showcase_duration
                    )
            else:
                print("❌ Showcase reframing failed.")
