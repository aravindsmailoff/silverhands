...
class NoAudioError(Exception):
    """The media has no audio track — nothing to transcribe (retained for backward compat)."""

_TRANSCRIPT_CACHE_DIR = os.environ.get("TRANSCRIPT_CACHE_DIR", ".transcript_cache")



def _has_audio_stream(media_path: str) -> bool:
    """Check if media file contains an audio stream using ffprobe."""
    try:
        cmd = [
            "ffprobe", "-v", "error",
            "-select_streams", "a",
...
def _file_hash(path: str) -> str:
    """Compute sha256 prefix of file content for cache keying."""
    try:
        h = hashlib.sha256()
...
def _transcribe_media_impl(media_path):
    """Core transcription logic without caching."""
    if not _has_audio_stream(media_path):
        print("🔇 [ASR] Video has no audio track — returning visual placeholder transcript.")
        return {
            "text": "",
            "language": "visual",
            "segments": [],
...
def transcribe_media(media_path):
    """Transcribe with configured backend, using file-hash cache if enabled."""
    cache_disabled = os.environ.get("DISABLE_TRANSCRIPT_CACHE", "0").strip() == "1"
    fhash = _file_hash(media_path) if not cache_disabled else ""
    backend = os.environ.get("TRANSCRIBE_BACKEND", "whisper").strip().lower()
    
    if fhash:
        os.makedirs(_TRANSCRIPT_CACHE_DIR, exist_ok=True)
        cache_file = os.path.join(_TRANSCRIPT_CACHE_DIR, f"{fhash}_{backend}.json")
        if os.path.exists(cache_file):
            try:
                with open(cache_file, "r", encoding="utf-8") as f:
                    cached = json.load(f)
                    print(f"⚡ [ASR cache] Loaded cached transcript for {os.path.basename(media_path)}")
                    return cached
            except Exception as e:
                print(f"⚠️ [ASR cache] Failed reading cache ({e}) — transcribing fresh")

    transcript = _transcribe_media_impl(media_path)
    ...
    if fhash:
        try:
            ...
            print(f"⚠️ [ASR cache] Failed writing cache: {e}")
